...
export async function POST(
  ...
  try {
    ...
    const idempotency = await ensureIdempotency(req, 'offdex_settle_request', auth.agentId, { intentId }, requestId);
    if (!idempotency.ok) {
    ...
    const update = await withTransaction(async (client) => {
      ...
      await client.query(
        `
        update offdex_settlement_intents
        set status = 'settling'::offdex_settlement_status, updated_at = now()
        where settlement_intent_id = $1
        `,
        [intentId]
      );
      ...
      await client.query(
        ...
        ) values ($1, $2, null, 'offdex.intent.settle_request', 'accepted', $3::jsonb, $4::jsonb, $5, now())
        `,
        [
          makeId('aud'),
          auth.agentId,
          JSON.stringify({ settlementIntentId: intentId, status: 'settling' }),
          JSON.stringify({}),
    ...
    if (!update.ok) {
      ...
      return errorResponse(
        409,
        {
          ...
          message: 'Off-DEX settle request requires ready_to_settle status.',
          actionHint: 'Submit maker/taker funding updates before requesting settlement.',
    ...
    const responseBody = { ok: true, settlementIntentId: intentId, status: 'settling' };
